...
export type HeaderProps = {
  ...
  /** When false, hides Sign In / Get Started (portfolio uses in-page nav only). Default true. */
  showAuthButtons?: boolean;
  /**
   * `contained` — centered card-style bar (default demo).
   * `wide` — full-width strip (e.g. portfolio shell).
   */
...
export function Header({
  ...
  const linkClass = (href: string) =>
    cn(
      ...
      activeHref && activeHref === href && "text-brand font-medium",
    );
  ...
  const navRow = (
    <nav
      className={cn(
        ...
        isWide ? "px-0" : "px-4",
        {
          "md:px-2": scrolled && !isWide,
        },
      )}
    >
      {wordmark ?? <WordmarkIcon className="h-4 shrink-0" />}
  ...
  return (
    <header
      className={cn(
        "z-50 w-full md:transition-all md:ease-out",
        /* Wide portfolio: fixed so the hero can paint from y=0 (no stacked “black strip” under the nav). */
        isWide
          ? cn(
              "fixed inset-x-0 top-0 max-w-none md:rounded-none md:shadow-none",
              open
                ? "border-b border-border bg-background/90 backdrop-blur-xl"
                : scrolled
                  ? "border-b border-white/[0.06] bg-background/90 backdrop-blur-xl supports-[backdrop-filter]:bg-background/70"
                  : "border-b border-transparent bg-transparent",
            )
          : cn(
              "sticky top-0 border-b border-transparent md:rounded-md md:border",
              "mx-auto max-w-5xl",
              scrolled && !open
                ? "border-border bg-background/95 backdrop-blur-lg supports-[backdrop-filter]:bg-background/50 md:shadow"
                : null,
              open ? "border-border bg-background/90" : null,
            ),
        className,
      )}
    >
      {isWide ? (
        <div className="mx-auto w-full max-w-[1800px] px-4 md:px-12 lg:px-16">
          {navRow}
        </div>
      ) : (
        navRow
      ...
      <div
        className={cn(
          "fixed top-14 right-0 bottom-0 left-0 z-50 flex flex-col overflow-hidden border-y bg-background/90 transition-opacity duration-200 ease-out md:hidden",
          open
      ...
      >
        <div
          ...
          className={cn(
            "flex h-full w-full flex-col justify-between gap-y-2 p-4 transition-transform duration-200 ease-out",
            ...
            {links.map((link) => (
              <a
                ...
                className={buttonVariants({
                  ...
                  className: cn(
                    "justify-start",
                    activeHref && activeHref === link.href && "text-brand font-medium",
                  ),
                })}
                href={link.href}
                onClick={(e) => handleLinkClick(e, link)}
...
export const WordmarkIcon = (props: React.ComponentProps<"svg">) => (
  <svg viewBox="0 0 84 24" fill="currentColor" aria-hidden {...props}>
